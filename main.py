...
def read_excel_mapping(excel_path: Path) -> Dict[str, Tuple[str, str]]:
    ...
    try:
        ...
        if file_ext == ".xls":
            preferred_engines: list[Literal["xlrd", "openpyxl"]] = [
                "xlrd",
                "openpyxl",
            ]  # .xls扩展名，但可能是xlsx格式
        ...
        if contract_col is None:
            raise ValueError(
                f"在Excel文件中找不到'合同编号'列。可用列: {list(df.columns)}"
            )
        if name_col is None:
...
def rename_pdf_files(
    target_dir: Path, excel_filename: str = "协商解除函签署名单-608人.xls"
):
    """
    批量重命名PDF文件

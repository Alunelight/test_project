...
def match_and_copy_pdfs(pdf_dir: Path, excel_path: Path, output_dir_name: str = "匹配结果"):
    """
    匹配PDF文件并复制到目标文件夹
    ...
    for pdf_file in pdf_files:
        ...
        if id_number not in excel_id_numbers:
            print(f"未匹配: {pdf_filename} (姓名: {name}, 身份证号: {id_number} 在Excel中未找到)")
            unmatched_count += 1
            continue
        ...
        try:
            ...
            if dest_path.exists():
                ...
                while dest_path.exists():
                    ...
                    counter += 1
                print(f"复制: {pdf_filename} -> {dest_path.name} (目标文件已存在，已重命名)")
            else:
